        ...
        <tbody>
            ...
            <tr v-for="(item, key) in pagination.paginator.data" :key="key">
                ...
                <td>
                    <div class="btns">
                        <router-link
                            :to="'/linea/' + item.id + '/edit'"
                            class="btn btn--green"
                        >
                            <i class="fas fa-edit"></i>
                        </router-link>
                        <router-link
                            :to="'/linea/' + item.id + '/copy'"
                            class="btn btn--yellow"
                        >
...
<script setup>
    ...
    const pagination = dataPaginator({
        urlBase: new URL(window.public_path + '/api/linea'),
    ...
    const deleteItem = (id) => {
        if (confirm('¿Está seguro de eliminar este registro?')) {
            httpRequest({
                url: window.public_path + '/api/linea/delete/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
    ...
    const restoreItem = (id) => {
        if (confirm('¿Está seguro de restaurar este registro?')) {
            httpRequest({
                url: window.public_path + '/api/linea/restore/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
