    ...
    <table>
        ...
        <tbody>            
            <tr v-for="(item, key) in pagination.paginator.data" :key="key">
                <td>{{ item.orden }}</td>
                <td style="background: gray;"><img v-bind:src="item.path" class="col-1" style="width: 50px;height: auto;" onerror="this.onerror=null;"></td>   
                <td>{{ item.name }}</td>
                <td>
                    <div class="btns">
                        <router-link
                            :to="'/adm/redes/' + item.id + '/edit'"
                            class="btn btn--green"
                        >
...
<script setup>
    ...
    const pagination = dataPaginator({
        urlBase: new URL(window.public_path + '/adm/redes'),
    ...
    const deleteItem = (id) => {
        if (confirm('¿Está seguro de eliminar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/redes/delete/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
    ...
    const restoreItem = (id) => {
        if (confirm('¿Está seguro de restaurar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/redes/restore/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
