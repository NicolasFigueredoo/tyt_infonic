        ...
        <tbody>
            ...
            <tr v-for="(item, key) in paginator.data" :key="key">
                ...
                <td class="btns d-flex justify-content-end">
                    <router-link
                        :to="'/adm/vendedores/' + item.id + '/edit'"
                        class="btn btn--green"
                    >
                        <i class="fas fa-edit"></i>
                    </router-link>
                    <!-- <router-link
                        :to="'/adm/vendedores/' + item.id + '/copy'"
                        class="btn btn--yellow"
                    >
...
<script setup>
    ...
    const deleteItem = (id) => {
        if (confirm('¿Está seguro de eliminar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/vendedores/delete/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
    ...
    const syncData = () => {
        ...
        let url = new URL(window.public_path + '/adm/vendedores')
