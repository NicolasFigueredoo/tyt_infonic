        ...
        <tbody>
            ...
            <tr v-for="(item, key) in pagination.paginator.data" :key="key">
                ...
                <td>
                    <div class="btns">
                        <router-link
                            :to="'/adm/empresa/slider/' + item.id + '/edit'"
                            class="btn btn--green"
                        >
...
<script setup>
    ...
    const pagination = dataPaginator({
        urlBase: new URL(window.public_path + '/adm/empresa/slider'),
    ...
    const deleteItem = (id) => {
        if (confirm('¿Está seguro de eliminar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/empresa/slider/delete/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
    ...
    const restoreItem = (id) => {
        if (confirm('¿Está seguro de restaurar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/empresa/slider/restore/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
