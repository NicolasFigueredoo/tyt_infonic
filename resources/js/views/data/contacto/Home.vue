...
<script setup>
    ...
    const pagination = dataPaginator({
        urlBase: new URL(window.public_path + '/adm/contacto'),
    ...
    const deleteItem = (id) => {
        if (confirm('¿Está seguro de eliminar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/contacto/delete/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
    ...
    const restoreItem = (id) => {
        if (confirm('¿Está seguro de restaurar este registro?')) {
            httpRequest({
                url: window.public_path + '/adm/contacto/restore/' + id,
                method: 'GET'
            })
            .then((data) => {
                pagination.syncData()
