...
function fetchClientes() {
  return axios.get(window.public_path + '/adm/metricas/clientes', {
    params: { tipo_evento: 'cliente_referrer' }
  })
...
function fetchData() {
  ...
  return axios.get(window.public_path + '/adm/metricas/referrer', { params })
    .then(res => {
      referrers.value = res.data.referrers;
      totals.value    = res.data.totals;
    })
    .catch(err => console.error(err));
}

watch([referrers, totals], () => {
...
function renderChart() {
  ...
  right: 20
    ...
    xAxis: {
      ...
      axisLabel: { rotate: 45, interval: 0, fontSize: 11, color: '#ECF0F1' }
    },
    yAxis: {
      type: 'value',
      axisLine: { show: false },
