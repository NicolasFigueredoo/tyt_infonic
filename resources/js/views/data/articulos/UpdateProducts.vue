<template>
    <SectionHeader>
        <template #title> {{ titulo }} Articulos Update </template>
        <template #buttons>
            <!--
            <button
                class="btn btn--gray"
            ...
            <router-link
                :to="'/adm/articulo/updateProducts/'"
                class="btn btn--yellow"
            >
            <i class="fas fa-database"></i> Actualizar productos
            </router-link>

            <router-link to="/adm/articulo/add" class="btn btn--green">
...
const restoreItem = (id) => {
    if (confirm("¿Está seguro de restaurar este registro?")) {
        httpRequest({
            url: window.public_path + "/adm/articulo/restore/" + id,
            method: "GET",
        })
            .then((data) => {
                pagination.syncData();
