...
class ContactoController extends Controller {
    private $model = Contacto::class;
    private $name = 'Contacto';
    private $prefixPermission = 'contacto';

    public function all() {
        // sniff($this->prefixPermission . '-*');
        $data = $this->model::where('seccion','=','contacto')->orderBy('id', 'desc');
    ...
    public function carrito() {
        // sniff($this->prefixPermission . '-*');
        $data = $this->model::where('seccion','=','carrito')->orderBy('id', 'desc');
        if ( request()->has('filters') && is_array(request()->filters) ) {
            foreach (request()->filters as $key => $value) {
                $data->where($key, 'like', '%'.$value.'%');
            }
    ...
    public function modal() {
        // sniff($this->prefixPermission . '-*');
        $data = $this->model::where('seccion','=','modal')->orderBy('id', 'desc');
        if ( request()->has('filters') && is_array(request()->filters) ) {
            foreach (request()->filters as $key => $value) {
                $data->where($key, 'like', '%'.$value.'%');
            }
    ...
    public function findCarrito() {
        $item = $this->model::where('seccion','=','carrito')->first();
        if ($item) {
            // $item->permissions = $item->permissions()->pluck('id');
            return $item;
        }
    ...
    public function findModal() {
        $item = $this->model::where('seccion','=','modal')->first();
        if ($item) {
            // $item->permissions = $item->permissions()->pluck('id');
            return $item;
        }
    ...
    public function store(Request $request, $id = null) {
        ...
        $item->wsp = $request->wsp;
        $item->horario = $request->horario;
        $item->mapa = $request->mapa;
    ...
    public function storeCarrito(Request $request, $id = null) {
        // store a new item
        
        $item = $this->model::where('seccion','=','carrito')->first();
                 
        $item->direccion = $request->informacion;
        $item->save();

        return $item;
    }
    public function storeModal(Request $request, $id = null) {
        $item = $this->model::where('seccion','=','modal')->first();
        ...
        if ($request->hasFile('imagenSrc')){
            ...
            $item->direccion  = 'inicio/' . $nombreImagen;

        }
         
        $item->orden = $request->activo;
        $item->mapa = $request->link;
